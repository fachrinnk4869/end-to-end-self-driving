...
class GPT_Seg(nn.Module):
    ...
    def forward(self, image_tensor, lidar_tensor, seg, velocity):
        ...
        token_embeddings = torch.cat(
            [image_tensor, lidar_tensor, seg_tensor], dim=1).permute(0, 1, 3, 4, 2).contiguous()
        token_embeddings = token_embeddings.view(
            bz, -1, self.n_embd)  # (B, an * T, C)
        ...
        image_tensor_out = x[:, :self.config.n_views*self.seq_len, :, :,
                             :].contiguous().view(bz * self.config.n_views * self.seq_len, -1, h, w)
        lidar_tensor_out = x[:, self.config.n_views*self.seq_len:self.config.n_views *
...
class Encoder(nn.Module):
    ...
    def forward(self, image_list, lidar_list, seg_list, velocity):
        ...
        seg_tensor = torch.stack(seg_list, dim=1).view(
            bz * self.config.n_views * self.config.seq_len, seg_channel, h, w)
        ...
        image_features_layer1, lidar_features_layer1 = self.transformer1(
            image_embd_layer1, lidar_embd_layer1, seg_embd_layer1, velocity)
