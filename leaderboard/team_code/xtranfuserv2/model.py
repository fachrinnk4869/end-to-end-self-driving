...
class x13(nn.Module):
    def __init__(self, config, device):
        ...
        self.tls_predictor = nn.Sequential(
            ...
            nn.Flatten(),
            nn.Linear(config.n_fmap_b3[5][-1], 2),
            nn.ReLU()
        )
    ...
    def forward(self, rgb_f, depth_f, next_route, velo_in):  # , gt_ss):
        ...
        sdc_features = self.sdc_encoder._model.conv1(top_view_sc)
        sdc_features = self.sdc_encoder._model.bn1(sdc_features)
        sdc_features = self.sdc_encoder._model.relu(sdc_features)
        sdc_features = self.sdc_encoder._model.maxpool(sdc_features)
        ...
        sdc_features = self.sdc_encoder._model.layer1(sdc_features)
        ...
        image_features_layer1, sdc_features_layer1 = self.transformer1(
        ...
        sdc_features = self.sdc_encoder._model.layer2(sdc_features)
        # fusion at (B, 128, 32, 32)
        image_embd_layer2 = self.avgpool(image_features)
        sdc_embd_layer2 = self.avgpool(sdc_features)
        image_features_layer2, sdc_features_layer2 = self.transformer2(
        ...
        sdc_features = self.sdc_encoder._model.layer3(sdc_features)
        # fusion at (B, 256, 16, 16)
        image_embd_layer3 = self.avgpool(image_features)
        sdc_embd_layer3 = self.avgpool(sdc_features)
        image_features_layer3, sdc_features_layer3 = self.transformer3(
        ...
        sdc_features = self.sdc_encoder._model.layer4(sdc_features)
        # fusion at (B, 512, 8, 8)
        image_embd_layer4 = self.avgpool(image_features)
        sdc_embd_layer4 = self.avgpool(sdc_features)
        image_features_layer4, sdc_features_layer4 = self.transformer4(
        ...
        image_features = image_features.view(
            bz, self.seq_len, -1)
        sdc_features = self.sdc_encoder._model.avgpool(sdc_features)
