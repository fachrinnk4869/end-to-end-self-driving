...
from x13_x13_swintv4.model import x13
from x13_x13_swintv4.config import GlobalConfig
from x13_x13_swintv4.data import scale_and_crop_image, scale_and_crop_image_cv, rgb_to_depth, swap_RGB2BGR, lidar_to_histogram_features, transform_2d_points
    ...
    def run_step(self, input_data, timestamp):
        ...
        if (self.step % 2 == 0 or self.step <= 4):
            ...
            for i, lidar_point_cloud in enumerate(self.input_buffer['lidar']):
                ...
                self.lidar_processed = lidar_transformed.to(
                    'cuda', dtype=torch.float32)

        pred_seg, pred_wp, psteer, pthrottle, pbrake, predl, pstops, pred_sc = self.net(
            self.input_buffer['rgb'], self.input_buffer['depth'], self.lidar_processed, target_point, gt_velocity)
        # forward pass
        # pred_seg, pred_wp, psteer, pthrottle, pbrake, predl, pstops, pred_sc = self.net(
