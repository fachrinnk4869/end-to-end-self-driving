...
class segmenter_x13_x13(nn.Module):
    ...
    def __init__(self, config, device):
        super(segmenter_x13_x13, self).__init__()
        self.config = config
        ...
        self.config_segmenter = './mmsegmentation/configs/segmenter/segmenter_vit-b_mask_8xb1-160k_ade20k-512x512.py'
        self.checkpoint_segmenter = './segmenter_x13_x13/segmenter_vit-b_mask_8x1_512x512_160k_ade20k_20220105_151706-bc533b08.pth'
        # Load model config
        self.model_segmenter = init_model(
            self.config_segmenter, self.checkpoint_segmenter)
        self.RGB_encoder = self.model_segmenter.backbone
        self.RGB_decoder = self.model_segmenter.decode_head
    ...
    def forward(self, rgb_f, depth_f, next_route, velo_in):
        ...
        if isinstance(RGB_features0, tuple):
            # Concatenate feature maps
            RGB_features1 = torch.cat(RGB_features0, dim=1)
        # print("output encoder: ", RGB_features0.shape)
        # ignore class token
        ...
        ss_f = self.final_ss_f(self.up(ss_f_0)).contiguous()
