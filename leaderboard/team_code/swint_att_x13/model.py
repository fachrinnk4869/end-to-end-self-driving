...
class Attention(nn.Module):
    def __init__(self, hidden_dim):
        ...
        # Linear layers to project inputs into query, key, and value
        self.query = nn.Linear(hidden_dim, hidden_dim)
        self.key = nn.Linear(hidden_dim, hidden_dim)
        self.value = nn.Linear(hidden_dim, hidden_dim)

    def forward(self, hx, ins):
        """
        Args:
            hx: Hidden state from GRU, shape (batch_size, hidden_dim)
            ins: Input features, shape (batch_size, feature_dim)
        Returns:
            attended_output: Output after attention, shape (batch_size, hidden_dim)
        """
        # Project inputs into query, key, and value
        q = self.query(hx)  # (batch_size, hidden_dim)
        k = self.key(ins)   # (batch_size, hidden_dim)
        v = self.value(ins)  # (batch_size, hidden_dim)

        # Compute attention scores
        # (batch_size, batch_size)
        scores = torch.matmul(q, k.transpose(-2, -1))
        scores = F.softmax(scores, dim=-1)  # Normalize scores

        # Apply attention to values
        attended_output = torch.matmul(scores, v)  # (batch_size, hidden_dim)
...
class swint_x13_x13(nn.Module):
    ...
    def __init__(self, config, device):
        ...
        self.checkpoint_file = './swint_att_x13/upernet_swin_base_patch4_window7_512x512_160k_ade20k_pretrain_224x224_22K_20210526_211650-762e2178.pth'
        # Load model config
        ...
        self.gru = nn.GRUCell(
            input_size=5, hidden_size=config.n_decoder[4][0])
    ...
    def forward(self, rgb_f, depth_f, next_route, velo_in):
        ...
        if isinstance(RGB_features0, list):
            ...
            RGB_features1 = torch.cat(
                [local_features_resized, global_features], dim=1)
        # print("output encoder: ", RGB_features0.shape)
        # ignore class token
        ...
        attention_layer = Attention(
            hidden_dim=5).to(self.gpu_device)
        ...
        for _ in range(self.config.pred_len):
            ...
            proj_ins = nn.Linear(
                hx.shape[1], ins.shape[1]).to(self.gpu_device)
            hx = proj_ins(hx)  # Project ins to match hx dimensions
            # Apply attention to combine `ins` and `hx`
            # print("ins", ins.shape)
            # print("hx", hx.shape)
            attended_hx = attention_layer(
                hx, ins)  # (batch_size, hidden_dim)

            # Pass the attended hidden state and input to the GRU
            hx = self.gru(ins, attended_hx)
