...
class PSPModule(nn.Module):
    def __init__(self, in_channels, pool_sizes):
        ...
        self.conv_layers = nn.ModuleList([
            nn.Conv2d(in_channels, in_channels, kernel_size=1) for _ in pool_sizes
        ])
...
class swint_x13_x13(nn.Module):
    ...
    def __init__(self, config, device):
        ...
        self.psp = PSPModule(in_channels=1024, pool_sizes=[1, 2, 3, 6])
    ...
    def forward(self, rgb_f, depth_f, next_route, velo_in):
        ...
        if isinstance(RGB_features0, list):
            ...
            psp_features = self.psp(RGB_features0[3])
            ...
            # Resize PSP features to match local_features
            psp_features_resized = F.interpolate(
                psp_features, size=local_features.shape[2:], mode='bilinear', align_corners=False)
            # Combine features
            RGB_features1 = torch.cat(
                [local_features, psp_features_resized], dim=1)
