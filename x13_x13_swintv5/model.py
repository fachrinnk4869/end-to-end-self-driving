...
class x13(nn.Module):
    def __init__(self, config, device):
        ...
        self.config_file = './mmsegmentation/configs/swin/swin-tiny-patch4-window7-in1k-pre_upernet_8xb2-160k_ade20k-512x512.py'
    ...
    def forward(self, rgb_f, depth_f, next_route, velo_in):  # , gt_ss):
        ...
        x, hw_shape = self.SC_encoder.patch_embed(top_view_sc)
        x = self.SC_encoder.drop_after_pos(x)
        for i, stage in enumerate(self.SC_encoder.stages):
        ...
        # print("SC_features8", SC_feature.shape)
        hx = self.necks_net(
            cat([RGB_features8, SC_feature], dim=1))
        # print(hx.shape)
