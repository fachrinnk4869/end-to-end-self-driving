...
class x13(nn.Module):
    def __init__(self, config, device):
        ...
        self.model_bb = init_model(
            self.config_file)
        self.SC_encoder = self.model_bb.backbone
        # Locate the first convolution layer
        old_conv = self.SC_encoder.patch_embed.projection  # Correct attribute

        # Create new convolution layer with matching device
        new_conv = nn.Conv2d(
            in_channels=23,  # Change from 3 to 23
        ...
        self.necks_net = nn.Sequential(  # inputnya dari 2 bottleneck
            nn.Conv2d(config.n_fmap_b3[4][-1]+config.n_fmap_b1[4][-1],
                      config.n_fmap_b3[4][1], kernel_size=1, stride=1, padding=0).to("cuda:0"),
            nn.AdaptiveAvgPool2d(1),
            nn.Flatten(),
            nn.Linear(config.n_fmap_b3[4][1],
                      config.n_fmap_b3[4][0]).to("cuda:0")
        )
        # ------------------------------------------------------------------------------------------------
        # wp predictor, input size 5 karena concat dari xy, next route xy, dan velocity
        self.gru = nn.GRUCell(
            input_size=5, hidden_size=config.n_fmap_b3[4][0]).to("cuda:0")
        self.pred_dwp = nn.Linear(config.n_fmap_b3[4][0], 2).to("cuda:0")
        # PID Controller
        ...
        self.controller = nn.Sequential(
            nn.Linear(config.n_fmap_b3[4][0],
                      config.n_fmap_b3[3][-1]).to("cuda:0"),
            nn.Linear(config.n_fmap_b3[3][-1], 3).to("cuda:0"),
            nn.ReLU()
        )
    ...
    def forward(self, rgb_f, depth_f, next_route, velo_in):  # , gt_ss):
        ...
        # bagian downsampling
        # print("top_view_sc", top_view_sc.shape)
        SC_features0 = self.SC_encoder(top_view_sc.to("cuda:0"))
        if isinstance(SC_features0, list):
            # Concatenate feature maps
            # local_features = RGB_features0[0]  # Stage 1 (1/4 resolution)
            # local_features1 = RGB_features0[1]  # Stage 2 (1/8 resolution)
            local_features1 = SC_features0[1]  # Stage 2 (1/8 resolution)
            ...
            local_features_resized1 = F.interpolate(
                local_features1, size=local_features2.shape[2:], mode='bilinear', align_corners=False)
            # local_features_resized2 = F.interpolate(
            #     local_features2, size=global_features.shape[2:], mode='bilinear', align_corners=False)
            # Combine features
            SC_features1 = torch.cat(
                [local_features_resized1, local_features2], dim=1)
        ...
        # print("RGB_features8", self.up(RGB_features8).shape)
        # print("SC_features8", SC_features1.shape)
        hx = self.necks_net(
            cat([RGB_features8.to("cuda:0"), SC_features1.to("cuda:0")], dim=1))
        # print(hx.shape)
        ...
        for _ in range(self.config.pred_len):
            ...
            ins = torch.cat([xy.to("cuda:0"), next_route.to("cuda:0"), torch.reshape(
                velo_in.to("cuda:0"), (velo_in.to("cuda:0").shape[0], 1))], dim=1)
            # print("ins", ins.shape)
            hx = self.gru(ins.to("cuda:0"), hx.to("cuda:0"))
            # print("hx", hx.shape)
            d_xy = self.pred_dwp(hx+tls_bias.to("cuda:0"))
            # print("d_xy", d_xy.shape)
            xy = xy.to("cuda:0") + d_xy
            # print("xy", xy.shape)
            out_wp.append(xy)
        ...
        # ------------------------------------------------------------------------------------------------
        # control decoder
        control_pred = self.controller(hx.to("cuda:0")+tls_bias.to("cuda:0"))
