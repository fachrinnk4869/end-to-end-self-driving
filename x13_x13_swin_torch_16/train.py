...
def BCEDice(Yp, Yt, smooth=1e-7):
    with torch.cuda.amp.autocast(enabled=False):
        ...
        Yp = Yp.view(-1)
        Yt = Yt.view(-1)
        # print("Yp dtype: ", Yp.dtype)
        # print("Yt dtype: ", Yt.dtype)
...
def train(data_loader, model, config, writer, cur_epoch, device, optimizer, params_lw, optimizer_lw):
    ...
    for data in data_loader:
        ...
        fronts = data['fronts'].to(device, dtype=torch.float16)
        depth_fronts = data['depth_fronts'].to(
            device, dtype=torch.float16)  # ambil yang terakhir aja #[-1]
        gt_velocity = data['velocity'].to(device, dtype=torch.float16)
        ...
        gt_waypoints = [torch.stack(data['waypoints'][i], dim=1).to(
            device, dtype=torch.float16) for i in range(config.seq_len, len(data['waypoints']))]
        gt_waypoints = torch.stack(gt_waypoints, dim=1).to(
            device, dtype=torch.float16)
        gt_steer = data['steer'].to(device, dtype=torch.float16)
        gt_throttle = data['throttle'].to(device, dtype=torch.float16)
        gt_brake = data['brake'].to(device, dtype=torch.float16)
        gt_red_light = data['red_light'].to(device, dtype=torch.float16)
        gt_stop_sign = data['stop_sign'].to(device, dtype=torch.float16)
