...
class xintra(nn.Module):
    def __init__(self, config, device):
        super(xintra, self).__init__()
        self.config = config
        ...
        # controller
        # MLP Controller
        self.projFeature = nn.Linear(
            config.n_fmap_b3[5][0], config.n_fmap_b3[4][0])
    ...
    def forward(self, rgb_f, depth_f, next_route, velo_in):  # , gt_ss):
        ...
        in_seg = self.decode_segmap_batch(ss_f)
        in_sdc = self.decode_segmap_batch(top_view_sc)

        # print("in_seg", in_seg.shape)
        x = {
            'rgb': in_rgb,
            'rgb_center': in_rgb,
            'target_point': next_route,
            'seg_image': in_seg,
            'sdc': in_sdc,
        ...
        hx, pred_wp, red_light, stop_sign = self.interfuser_model(
            x)
        red_light = red_light[:, 0]
        stop_sign = stop_sign[:, 0]
        # ------------------------------------------------------------------------------------------------
        # control decoder
        # print("hx", hx.view(hx.size(0), -1)  # [1, 400, 256] → [1, 102400]
        #       .shape)
        # print("tls_bias", tls_bias.shape)
        hx = hx.reshape(hx.size(0), -1)
        hx = self.projFeature(hx)
        control_pred = self.controller(hx+tls_bias)
        # print("control_pred", control_pred.shape)
    ...
    def decode_segmap_batch(self, seg_batch):
        """
        Convert [B, C, H, W] prediction to [B, 3, H, W] RGB using colormap.
        """
        seg_batch = seg_batch.cpu().detach()  # [B, C, H, W]
        pred = torch.argmax(seg_batch, dim=1)  # [B, H, W]
        B, H, W = pred.shape
        # [num_classes, 3]
        cmap = torch.tensor(self.config.SEG_CLASSES['colors'])

        # pastikan colormap di CPU dan float
        cmap = cmap.float()

        # [B, H, W] → [B, H, W, 3]
        rgb_batch = torch.zeros((B, H, W, 3), dtype=torch.float)
        for cls_id in range(len(self.config.SEG_CLASSES['colors'])):
            rgb_batch[pred == cls_id] = cmap[cls_id]

        # [B, H, W, 3] → [B, 3, H, W]
        rgb_batch = rgb_batch.permute(0, 3, 1, 2)
        return rgb_batch.to(self.gpu_device)
    ...
    def show_seg_sdc(self, seg, sdc):
        ...
        # print("imgx2", imgx2.shape)
        cv2.imshow("seg bro", imgx.astype(np.uint8))
        cv2.imshow("bev bro", imgx2.astype(np.uint8))
        cv2.waitKey(1)
        seg = imgx.astype(np.uint8)
        sdc = imgx2.astype(np.uint8)
        seg = torch.from_numpy(seg).permute(
            2, 0, 1).float().to(self.gpu_device)  # from [C, H, W] to [H, W, C]
        sdc = torch.from_numpy(sdc).permute(
            2, 0, 1).float().to(self.gpu_device)
        return seg, sdc
