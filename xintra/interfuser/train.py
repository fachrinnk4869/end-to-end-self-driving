...
parser.add_argument("--localrank", default=0, type=int)
parser.add_argument(
...
def main():
    ...
    args.localrank = int(os.environ.get("LOCAL_RANK", 0))
    if args.log_wandb:
        if has_wandb:
    ...
    if args.distributed:
        args.device = "cuda:%d" % args.localrank
    ...
    data_config = resolve_data_config(
        vars(args), model=model, verbose=args.localrank == 0
    )
    ...
    if args.distributed:
        if has_apex and use_amp != "native":
            ...
            model = NativeDDP(
                model, device_ids=[args.localrank], find_unused_parameters=True
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            output = model(input)
            loss_traffic, loss_velocity = loss_fns["traffic"](
                output[0], target[4])
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.distributed:
                ...
                reduced_loss_traffic = reduce_tensor(
                    loss_traffic.data, args.world_size)
                ...
                losses_velocity.update(
                    reduced_loss_velocity.item(), batch_size)

                reduced_loss_waypoints = reduce_tensor(
                    loss_waypoints.data, args.world_size
                )
                losses_waypoints.update(
                    reduced_loss_waypoints.item(), batch_size)
                reduced_loss_junction = reduce_tensor(
                    loss_junction.data, args.world_size
                )
                losses_junction.update(
                    reduced_loss_junction.item(), batch_size)
                ...
                losses_stop_sign.update(
                    reduced_loss_stop_sign.item(), batch_size)
                if writer and args.localrank == 0:
                    writer.add_scalar(
                        "train/loss", reduced_loss.item(), num_updates)
                    writer.add_scalar(
                    ...
                    # Add Image
                    writer.add_image(
                        "train/front_view", retransform(
                            input["rgb"][0]), num_updates
                    )
                    writer.add_image(
                        "train/left_view",
                    ...
                    writer.add_image(
                        "train/pred_traffic",
                        torch.clip(output[0][0], 0, 1).view(
                            1, 20, 20, 7)[:, :, :, 0],
                        num_updates,
                    )
                    writer.add_image(
                    ...
                    input["lidar"][0] = input["lidar"][0] / \
                        torch.max(input["lidar"][0])
                    writer.add_image(
                        "train/lidar", torch.clip(input["lidar"]
                                                  [0], 0, 1), num_updates
                    )
                    writer.add_image(
                        "train/gt_traffic",
                        torch.clip(target[4][0], 0, 1).view(
                            1, 20, 20, 7)[:, :, :, 0],
                        num_updates,
                    )
                    writer.add_image(
...
def validate(
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if args.distributed:
                ...
                reduced_junction_error = reduce_tensor(
                    junction_error, args.world_size)
        ...
        if writer:
            ...
            writer.add_scalar("val/loss_waypoints",
                              losses_waypoints.avg, epoch)
            ...
            writer.add_scalar("val/loss_stop_sign",
                              losses_stop_sign.avg, epoch)
    ...
    metrics = OrderedDict(
        [("loss", losses_m.avg), ("l1_error", l1_errorm.avg)])
