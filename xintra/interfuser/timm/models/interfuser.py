...
class SpatialSoftmax(nn.Module):
    def __init__(self, height, width, channel, temperature=None, data_format="NCHW"):
        ...
        pos_x, pos_y = np.meshgrid(
            np.linspace(-1.0, 1.0, self.height), np.linspace(-1.0,
                                                             1.0, self.width)
        )
...
class MultiPath_Generator(nn.Module):
    ...
    def forward(self, x, measurements):
        mask = measurements[:, :6]
        mask = mask.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).repeat(1,
                                                                     1, 1, 100, 100)
...
class LinearWaypointsPredictor(nn.Module):
    def __init__(self, input_dim, cumsum=True):
        ...
        self.head_fc1_list = nn.ModuleList(
            [nn.Linear(input_dim, 64) for _ in range(6)])
        self.head_relu = nn.ReLU(inplace=True)
        self.head_fc2_list = nn.ModuleList(
            [nn.Linear(64, 2) for _ in range(6)])
...
class GRUWaypointsPredictor(nn.Module):
    def __init__(self, input_dim, waypoints=3):
        super().__init__()
...
class GRUWaypointsPredictorWithCommand(nn.Module):
    def __init__(self, input_dim, waypoints=10):
        super().__init__()
        # self.gru = torch.nn.GRUCell(input_size=input_dim, hidden_size=64)
        self.grus = nn.ModuleList([torch.nn.GRU(
            input_size=input_dim, hidden_size=64, batch_first=True) for _ in range(6)])
        self.encoder = nn.Linear(2, 64)
...
class Interfuser(nn.Module):
    def __init__(
        ...
        if use_different_backbone:
            ...
            lidar_embed_layer = partial(
                HybridEmbed, backbone=self.lidar_backbone)
        ...
        if self.end2end:
            ...
            self.query_embed = nn.Parameter(
                torch.zeros(400 + 11, 1, embed_dim))
        ...
        if self.end2end:
            ...
            self.waypoints_generator = GRUWaypointsPredictorWithCommand(
                embed_dim)
            ...
            self.waypoints_generator = LinearWaypointsPredictor(
                embed_dim, cumsum=True)

        self.junction_pred_head = nn.Linear(embed_dim, 2)
        self.traffic_light_pred_head = nn.Linear(embed_dim, 1)
        self.stop_sign_head = nn.Linear(embed_dim, 1)
    ...
    def forward_features(
        ...
        front_image_token, front_image_token_global = self.rgb_patch_embed(
            front_image)
        ...
        seg_image_token, seg_image_token_global = self.rgb_patch_embed(
            seg_image
        )
        if self.use_view_embed:
            seg_image_token = (
                seg_image_token
                + self.view_embed[:, :, 2:3, :]
                + self.position_encoding(seg_image_token)
            )
        else:
            seg_image_token = seg_image_token + self.position_encoding(
                seg_image_token
            )
        seg_image_token = seg_image_token.flatten(2).permute(2, 0, 1)
        seg_image_token_global = (
            seg_image_token_global
            + self.view_embed[:, :, 2, :]
            + self.global_embed[:, :, 2:3]
        )
        seg_image_token_global = seg_image_token_global.permute(
            2, 0, 1)
        ...
        sdc_token, sdc_token_global = self.lidar_patch_embed(sdc)
        if self.use_view_embed:
            sdc_token = (
                sdc_token
                + self.view_embed[:, :, 4:5, :]
                + self.position_encoding(sdc_token)
            )
        else:
            sdc_token = sdc_token + self.position_encoding(sdc_token)
        sdc_token = sdc_token.flatten(2).permute(2, 0, 1)
        sdc_token_global = (
            sdc_token_global
            + self.view_embed[:, :, 4, :]
            + self.global_embed[:, :, 4:5]
        )
        sdc_token_global = sdc_token_global.permute(2, 0, 1)
        features.extend([sdc_token, sdc_token_global])
        # for i, f in enumerate(features):
        #     print(f"Tensor {i} shape: {f.shape}")
    ...
    def forward(self, x):
        ...
        hs = self.decoder(self.query_embed.repeat(
            1, bs, 1), memory, query_pos=tgt)[0]
        ...
        waypoints_feature = hs[:, 401:404]
        waypoints = self.waypoints_generator(
            waypoints_feature, target_point)
        ...
        velocity = measurements.contiguous(
        ).view(-1, 1, 1)    # dari [1] → [1, 1, 1]
        velocity = velocity.repeat(1, 400, 32)
        # print("traffic_feature", traffic_feature.shape)
        # print("velocity", velocity.shape)
        traffic_feature_with_vel = torch.cat(
            [traffic_feature, velocity], dim=2)
        traffic = self.traffic_pred_head(traffic_feature_with_vel)
        # print("traffic", traffic.shape)
        # traffic_light_state = traffic_light_state[0]
        # stop_sign = stop_sign[0]
        return traffic_feature, waypoints, traffic_light_state, stop_sign
