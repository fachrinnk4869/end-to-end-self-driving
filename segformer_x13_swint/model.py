...
class x13(nn.Module):
    def __init__(self, config, device):
        ...
        self.config_file = './mmsegmentation/configs/segformer/segformer_mit-b0_8xb1-160k_cityscapes-1024x1024.py'

        self.checkpoint_file = './segformer_mit-b0_8x1_1024x1024_160k_cityscapes_20211208_101857-e7f88502.pth'
        # Load model config
        self.model_bb = init_model(
            self.config_file, self.checkpoint_file)
        self.RGB_encoder = self.model_bb.backbone
        # print("encoder", self.RGB_encoder)
        self.RGB_decoder = self.model_bb.decode_head
        self.RGB_decoder.cls_seg = nn.Sequential()
        # SS
        self.up = nn.Upsample(
            scale_factor=2, mode='bilinear', align_corners=True)
        self.conv3_ss_f = ConvBlock(
            channel=[config.n_fmap_b3[4][-1], config.n_fmap_b3[3][-1]])
        self.conv2_ss_f = ConvBlock(
            channel=[config.n_fmap_b3[3][-1], config.n_fmap_b3[2][-1]])
        self.conv1_ss_f = ConvBlock(
            channel=[config.n_fmap_b3[2][-1], config.n_fmap_b3[1][-1]])
        self.conv0_ss_f = ConvBlock(
            channel=[config.n_fmap_b3[1][-1], config.n_fmap_b3[0][0]])
    ...
    def forward(self, rgb_f, depth_f, next_route, velo_in):  # , gt_ss):
        ...
        if isinstance(RGB_features0, list):
            local_features2 = RGB_features0[3]  # Stage 34 (1/32 resolution)
            RGB_features1 = local_features2
        # print("rg_features_7", RGB_features7.shape)
        # bagian upsampling

        inputs = self.RGB_decoder._transform_inputs(RGB_features0)
        outs = []
        for idx in range(len(inputs)):
            x = inputs[idx]
            conv = self.RGB_decoder.convs[idx]
            outs.append(
                resize(
                    input=conv(x),
                    size=inputs[0].shape[2:],
                    mode=self.RGB_decoder.interpolate_mode,
                    align_corners=self.RGB_decoder.align_corners))

        ss_f_4 = self.RGB_decoder.fusion_conv(torch.cat(outs, dim=1))
        # print("output ss_f_4: ", ss_f_4.shape)
        ss_f_3 = self.conv3_ss_f(ss_f_4)
        # print("output ss_f_3: ", ss_f_3.shape)
        # print("self.up(ss_f_3)", self.up(ss_f_3).shape)
        ...
        hx = self.necks_net(cat([RGB_features1, SC_features7], dim=1))
        # print(hx.shape)
