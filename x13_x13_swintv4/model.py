...
class x13(nn.Module):
    def __init__(self, config, device):
        ...
        new_conv = nn.Conv2d(
            in_channels=25,  # Change from 3 to 23
        ...
        self.controller = nn.Sequential(
            nn.Linear(config.n_fmap_b3[4][0],
                      config.n_fmap_b3[3][-1]),
            nn.Linear(config.n_fmap_b3[3][-1], 3),
            nn.ReLU()
        )

    # , gt_ss):
    def forward(self, rgb_f, depth_f, lidar_f, next_route, velo_in):
        # ------------------------------------------------------------------------------------------------
        # print("rgb_in: ", rgb_f.shape)
        # print(depth_f.shape)
        # print("lidar_in: ", lidar_f.shape)
        ...
        SC_features0 = self.SC_encoder(cat([top_view_sc, lidar_f], dim=1))
        if isinstance(SC_features0, list):
            ...
            local_features1 = SC_features0[0]  # Stage 1 (1/8 resolution)
        ...
        hx = self.necks_net(
            cat([RGB_features8, SC_features1], dim=1))
        # print(hx.shape)
