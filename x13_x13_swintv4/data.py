                ...
                for route in routes:
                    ...
                    for seq in range(num_seq):
                        ...
                        for i in range(self.seq_len):
                            ...
                            depth_fronts.append(
                                route_dir+"/depth_front/"+filename)
                            # lidar
                            # point cloud
                            lidars.append(
        ...
        for i in range(self.seq_len + self.pred_len):
            ...
            if i < self.seq_len:
                ...
                lidar_processed = lidar_to_histogram_features(
                    full_lidar[i], crop=self.config.input_resolution)
                data['lidars'] = lidar_processed
...
def lidar_to_histogram_features(lidar, crop=256):
    ...
    features = features[:, start:end, start:end]  # [2, 244, 244]
    return features
...
def scale_and_crop_image_cv(image, scale=1, crop=256):
    upper_left_yx = [int((image.shape[0]/2) - (crop/2)),
                     int((image.shape[1]/2) - (crop/2))]
    cropped_im = image[upper_left_yx[0]:upper_left_yx[0] +
                       crop, upper_left_yx[1]:upper_left_yx[1]+crop, :]
...
def cls2one_hot(ss_gt):
    ...
    result = (np.arange(23) == ss_gt[..., None]).astype(
        int)  # len(classes_ss_su
...
def rgb_to_depth(de_gt):
    ...
    normalized_depth = np.dot(arrayd, [65536.0, 256.0, 1.0])
    # (256.0 * 256.0 * 256.0 - 1.0) --> rangenya 0 - 1
    depthx = normalized_depth/16777215.0
